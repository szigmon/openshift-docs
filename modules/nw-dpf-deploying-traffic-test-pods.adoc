...
[NOTE]
====
These test workloads use the `nicolaka/netshoot` container image, a community networking-troubleshooting image that is not officially supported by Red Hat or NVIDIA.
Use it only for connectivity validation and testing, not in production workloads.
====
