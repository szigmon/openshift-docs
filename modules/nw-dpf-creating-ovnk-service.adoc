...
= Create the HBN and OVN-Kubernetes DPU service configurations
...
[NOTE]
====
HBN and OVN-Kubernetes are the supported DPU network services. The DOCA Telemetry Service (DTS), which you configure in a later step, is deployed for observability and is not a network service.
====

[NOTE]
====
The `DPUServiceTemplate` resources for both HBN and OVN-Kubernetes are automatically created and managed by the `dpf-hcp-provisioner-operator`.
