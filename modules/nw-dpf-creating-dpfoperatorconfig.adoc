...
. Verify that all Operator deployments in the `dpf-operator-system` namespace become available:
+
[NOTE]
====
...
[source,terminal]
----
$ oc wait --for=condition=Available deployment --all -n dpf-operator-system --timeout=300s
----
+
.Example output
[source,terminal]
----
deployment.apps/dpf-operator-controller-manager condition met
deployment.apps/dpf-provisioning-controller-manager condition met
deployment.apps/dpuservice-controller-manager condition met
----
