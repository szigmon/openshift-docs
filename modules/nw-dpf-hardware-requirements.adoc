...
[role="_abstract"]
A DPF v{dpf-version} deployment on {product-title} {product-version} requires a workstation with CLI tools, a management cluster, at least two worker servers with NVIDIA BlueField-3 DPUs, dedicated management and DPU network switches, and a shared storage server for BFB images.
