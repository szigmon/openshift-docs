...
If you configured jumbo frames (the MTU 9000 `DPUFlavor`), add `--set networkMTU=9000` to the command to match the node MTU. Omit that flag when using the standard MTU of `1500`.
+
[source,terminal]
----
