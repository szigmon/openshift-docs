...
* The Helm CLI (`helm`) is installed on your workstation and authenticated to `registry.redhat.io`. Because Helm stores its own registry credentials separately from the container runtime, you must log in with Helm directly:
+
[source,terminal]
----
