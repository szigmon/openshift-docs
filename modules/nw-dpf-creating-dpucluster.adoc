...
The `spec.kubeconfig` field references the name of the secret that holds the hosted cluster admin kubeconfig. After the hosted cluster is created, the DPF HCP Provisioner Operator automatically creates this secret in the same namespace; you do not create it manually.
