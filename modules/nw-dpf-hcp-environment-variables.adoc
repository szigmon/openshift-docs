...
|`BLUEFIELD_OCP_IMAGE`
|The BlueField {product-title} layer container image URL. When set, this value is used as `machineOSURL` in the `DPFHCPProvisioner` resource and skips automatic image lookup. This image will be available on registry.redhat.io for GA release.
